...
public class GetUrlTask extends AsyncTask<String, Void, Void> {
  ...
  static public String PrintExtraParams(String extra_params) {
    ArrayList<String> fields =
        new ArrayList<String>(Arrays.asList(extra_params.split(ShoppingUtils.TASK_CART_SEPARATOR)));
    if (fields.size() != 2) return extra_params;
    String to_return = "Task: " + fields.get(0) + ", Line Item: ";
    ShoppingUtils.LineItem item = ShoppingUtils.ParseLineItem(fields.get(1));
    to_return += item == null ? "NULL" : ShoppingUtils.PrintLineItemHumanReadable(item);
    return to_return;
  }
