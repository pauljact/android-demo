...
public class ProductsAdapter extends ArrayAdapter<ProductsPageParser.ProductItem> implements AdapterWithImages {
	...
	public static void SetPrice(ProductsPageParser.ProductItem product,
      ...
      String bux = "";
      if (!isOriginalPrice && product.bux_ != null) bux = product.bux_;
	  if (isOriginalPrice && product.orig_bux_ != null) bux = product.orig_bux_;
      String usd = "";
      if (!isOriginalPrice && product.usd_ != null) usd = product.usd_;
	  if (isOriginalPrice && product.orig_usd_ != null) usd = product.orig_usd_;
      String points = "";
      if (!isOriginalPrice && product.points_ != null) points = product.points_;
	  if (isOriginalPrice && product.orig_points_ != null) points = product.orig_points_;
      ...
      if (!usd.isEmpty()) {
    	...
    	if (isOriginalPrice) {
		  ...
		  holder.jact_price_usd_.setText(parent_activity.getString(R.string.jact_price_str) + " ");
          holder.jact_price_point_icon_.setVisibility(View.VISIBLE);
		  holder.jact_price_second_line_.setVisibility(View.GONE);
		  holder.jact_price_points_.setVisibility(View.VISIBLE);
      	  holder.jact_price_points_.setText(NumberFormat.getNumberInstance(Locale.US).format(Integer.parseInt(points)));
    	}
      } else {
